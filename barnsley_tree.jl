...
# This is a huge library that takes a gajillion years to compile.
# Be patient...
import Plots
...
"""
Plots the tree generated by `generate_tree_`.
"""
function plot_tree_(image_array)
    # Conversion to RGB values
    image = map(x -> Colors.RGB(0.0,x,0.0), image_array)
    Images.save(IMAGE_PATH, image)
end
...
function main()
    ...
    @time trimmed_array = trim_tree_(image_array)
    println("Plotting the image...")
    @time plot_tree_(trimmed_array)
    println("Done.")
end
